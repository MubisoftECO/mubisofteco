...
public interface ProductTypeRepository extends JpaRepository<ProductType, Long> {
    ...
    @Query("select p from ProductType p where p.name_en like concat(?1, '%')")
    List<ProductType> getAllByName_enStartingWith(String filter);

    @Query("select p from ProductType p where p.name_es like concat(?1, '%')")
    List<ProductType> getAllByName_esStartingWith(String filter);

    @Query("select p from ProductType p where p.name_eu like concat(?1, '%')")
